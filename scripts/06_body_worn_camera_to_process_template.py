...
BUILD = os.path.join(WORK, 'build6')
...
TITLE = 'Body Worn Camera User Guidelines'
VERSION_LABEL = 'Version 3.0'
...
def heading(p_src, style, bookmark=False):
    # literal numbers are stripped; the template's own numbering supplies them
...
# ---------------------------------------------------------------- classify + convert
H1 = {45, 47, 49, 52, 61, 69, 73, 79, 81, 83, 88, 90, 92, 101, 108}
H2 = {84, 86}
GOV_TBL, HIST_TBL = 24, 27
TABLES = {91, 109}                       # roles, definitions
BODY_START = 45                          # everything before this is old front matter

out = []
governance_table = history_table = None
for i, el in enumerate(iblocks):
    ...
    if i == GOV_TBL:
        ...
        continue
    if i == HIST_TBL:
        history_table = transform_table(el, mode='header-row')
        continue
    if i < BODY_START:                    # old cover, contact page, typed ToC
        continue
    if i in TABLES:
    ...
    if i in H1:
        ...
        TOC_ENTRIES.append(('n', NUM_PREFIX.sub('', text).strip(), bm))
        out.append(p)
    elif i in H2:
        p, _ = heading(el, 'Heading2')
        out.append(p)
    else:
        # 'List Bullet' style carried no numbering in the source; attach the
        # template's bullet list so these render as the lists they were meant to be
        out.append(body_para(el, bullet=(style_name == 'ListBullet')))
...
keep.append(governance_table)                 # source's governance table (real values)
keep.append(tblocks[28])                      # spacer
keep.append(tblocks[29])                      # 'Document History' heading
keep.append(history_table)                    # source's history table (real values)
...
out_docx = os.path.join(WORK, 'out6.docx')
